...
class Interfacer():
    ...
    def ompl_find_pose(
        ...
        if "pick" in affordance:
            ...
            goal_pose[1] += (1.0 if pour_direction == 'left' else -1.0) * (src_obj_height * 0.75) + (tgt_obj_width * 0.5)

            # -- we want the object to be placed somewhere above the target container:
            goal_pose[2] += (tgt_obj_height) + (src_obj_height * 0.5)
