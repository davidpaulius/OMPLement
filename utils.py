...
class Interfacer():
    ...
    def move_to_configs(self, path: list[float]) -> list[float]:
        ...
        for P in range(len(path)):
            ...
            new_pose = self.sim.getObjectPose(self.sim.getObject(f'/{self.robot_name}/tip'), self.sim.getObject(f'/{self.robot_name}'))
    ...
    def pour(
        ...
        while True:
            result = self.ompl_execute(
                ...
                target_pose=self.ompl_get_target_pose(
                    target_object=target_container,
                    affordance='pour',
                )[0],
                ompl_args=ompl_args,
